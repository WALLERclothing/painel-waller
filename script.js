...
+// ==========================================
+const OFFLINE_QUEUE_KEY = 'waller_offline_queue_v1';
+let filaOffline = [];
+
+function carregarFilaOffline() {
+    try { filaOffline = JSON.parse(localStorage.getItem(OFFLINE_QUEUE_KEY) || '[]'); }
+    catch { filaOffline = []; }
+    atualizarIndicadorConexao();
+}
+
+function salvarFilaOffline() {
+    localStorage.setItem(OFFLINE_QUEUE_KEY, JSON.stringify(filaOffline));
+    atualizarIndicadorConexao();
+}
+
+function comTimestampServidor(data) {
+    return Object.fromEntries(Object.entries(data).map(([k, v]) => [k, v === '__SERVER_TIMESTAMP__' ? firebase.firestore.FieldValue.serverTimestamp() : v]));
+}
+
+function comIncrementos(data) {
+    let result = {};
+    Object.entries(data).forEach(([k, v]) => {
+        if(v && typeof v === 'object' && v.__op === 'increment') result[k] = firebase.firestore.FieldValue.increment(v.by || 0);
+        else result[k] = v;
+    });
+    return result;
+}
+
+async function executarOperacaoFirestore(op) {
+    if(op.type === 'set') {
+        await db.collection(op.collection).doc(op.docId).set(comTimestampServidor(comIncrementos(op.data || {})), op.options || {});
+        return;
+    }
+    if(op.type === 'update') {
+        await db.collection(op.collection).doc(op.docId).update(comTimestampServidor(comIncrementos(op.data || {})));
+        return;
+    }
+    if(op.type === 'delete') {
...
+            if(item.type === 'set') batch.set(ref, comTimestampServidor(comIncrementos(item.data || {})), item.options || {});
+            if(item.type === 'update') batch.update(ref, comTimestampServidor(comIncrementos(item.data || {})));
...
+}
+
+async function enfileirarOuExecutar(op) {
+    if(!navigator.onLine) {
+        filaOffline.push(op); salvarFilaOffline();
+        showToast('Sem internet: operação salva na fila offline.');
+        return { queued: true };
+    }
+    try {
+        await executarOperacaoFirestore(op);
+        return { queued: false };
+    } catch (e) {
+        filaOffline.push(op); salvarFilaOffline();
+        showToast('Falha de conexão: operação enfileirada para sincronizar.', true);
+        return { queued: true };
+    }
+}
+
+async function sincronizarFilaOffline() {
+    if(!navigator.onLine) { showToast('Sem internet para sincronizar.', true); return; }
+    if(!filaOffline.length) { showToast('Fila offline vazia.'); return; }
+
+    let pendentes = [...filaOffline];
+    filaOffline = [];
+    let sincronizadas = 0;
+
...
+    }
+    salvarFilaOffline();
+    showToast(`${sincronizadas} operação(ões) sincronizadas. ${filaOffline.length ? `Pendentes: ${filaOffline.length}` : ''}`.trim(), filaOffline.length > 0);
+}
+
+function atualizarIndicadorConexao() {
+    const badge = document.getElementById('statusOffline');
+    const btnSync = document.getElementById('btnSyncOffline');
+    if(!badge || !btnSync) return;
+
+    if(navigator.onLine) {
+        badge.classList.remove('offline'); badge.classList.add('online');
+        badge.innerText = filaOffline.length ? `🟡 ONLINE | FILA: ${filaOffline.length}` : '🟢 ONLINE';
+    } else {
+        badge.classList.remove('online'); badge.classList.add('offline');
+        badge.innerText = `🔴 OFFLINE | FILA: ${filaOffline.length}`;
+    }
+    btnSync.style.display = filaOffline.length ? 'inline-flex' : 'none';
+}
+
+window.addEventListener('online', () => { atualizarIndicadorConexao(); sincronizarFilaOffline(); });
+window.addEventListener('offline', () => { atualizarIndicadorConexao(); showToast('Você está offline. Modo fila ativado.', true); });
+
+try {
...
+
+function ajustarEstoqueRapido(id, delta) {
+    enfileirarOuExecutar({ type: 'update', collection: 'estampas', docId: id, data: { estoque: { __op: 'increment', by: delta } } });
+}
+
+function autocompletarEstampa(val) {
+    let code = val.toUpperCase().trim();
...
+    let docId = document.getElementById('editEstampaCodigoOriginal').value || cod;
+
+    enfileirarOuExecutar({ type: 'set', collection: 'estampas', docId: docId, data: { codigo: docId, nome: nom, categoria: cat, estoque: est }, options: { merge: true } })
+    .then(() => { cancelarEdicaoEstampa(); showToast("Estampa Salva!"); });
+}
+
+function excluirEstampa(id) { if(confirm(`Apagar estampa?`)) enfileirarOuExecutar({ type: 'delete', collection: 'estampas', docId: id }); }
+
+function prepararEdicaoEstampa(c, n, e, cat) {
+    document.getElementById('cadCodigoEstampa').value = c; document.getElementById('cadCodigoEstampa').disabled = true;
+    document.getElementById('cadNomeEstampa').value = n; document.getElementById('cadEstoque').value = e;
...
+function salvarFichaCliente() {
+    let w = document.getElementById('fichaWhatsapp').value;
+    enfileirarOuExecutar({ type: 'set', collection: 'clientes', docId: w, data: {
+        whatsapp: w, nome: document.getElementById('fichaNome').value.toUpperCase(),
+        insta: document.getElementById('fichaInsta').value, dataNasc: document.getElementById('fichaDataNasc').value,
+        cep: document.getElementById('fichaCEP').value, endereco: document.getElementById('fichaEndereco').value,
+        obs: document.getElementById('fichaObs').value
+    }, options: {merge: true} }).then(() => { showToast("Ficha do Cliente Salva!"); fecharFichaCliente(); });
+}
+
+// ==========================================
+// CARRINHO E LANÇAMENTO DE PEDIDO
...
+
+    try {
+        const pedidoId = `P${Date.now()}${Math.floor(Math.random() * 1000)}`;
+        const ops = [{
+            type: 'set', collection: 'pedidos', docId: pedidoId,
+            data: {
+                numeroPedido: numGerado, nome: nome, whatsapp: whatsapp, cep: cep, endereco: end, valorFrete: frete,
+                valorTotal: total, metodoPagamento: document.getElementById('metodoPagamento').value, statusPagamento: document.getElementById('statusPagamento').value,
+                itens: carrinhoTemporario, status: 'PEDIDO FEITO', dataCriacao: '__SERVER_TIMESTAMP__'
+            }
+        }, {
+            type: 'set', collection: 'clientes', docId: whatsapp,
+            data: { whatsapp: whatsapp, nome: nome, cep: cep, endereco: end }, options: { merge: true }
+        }];
+
+        carrinhoTemporario.forEach(item => {
+            if(item.codigoEstampa && catalogoEstampas[item.codigoEstampa]) {
+                ops.push({ type: 'update', collection: 'estampas', docId: item.codigoEstampa, data: { estoque: { __op: 'increment', by: -item.quantidade } } });
+            }
+        });
+
+        const resultado = await enfileirarOuExecutar({ type: 'batch', ops });
+
+        document.getElementById('nome').value = ''; document.getElementById('whatsapp').value = ''; document.getElementById('cep').value=''; document.getElementById('endereco').value=''; document.getElementById('numeroEnd').value=''; document.getElementById('valorFrete').value=''; document.getElementById('valorTotal').value=''; document.getElementById('alertaClienteFiel').style.display = 'none';
+        carrinhoTemporario = []; atualizarTelaCarrinho(); document.getElementById('btnGerarOrdem').innerText = "GERAR ORDEM DE SERVIÇO";
+        showToast(resultado.queued ? `Pedido #${numGerado} salvo na fila offline.` : `PEDIDO #${numGerado} SALVO!`);
+    } catch (e) { showToast("Erro ao salvar", true); }
+}
+
+// ==========================================
...
+function drag(ev) { ev.dataTransfer.setData("text", ev.target.id); }
+function allowDrop(ev) { ev.preventDefault(); }
+function drop(ev, novoStatus) { ev.preventDefault(); enfileirarOuExecutar({ type: 'update', collection: 'pedidos', docId: ev.dataTransfer.getData("text"), data: { status: novoStatus } }); showToast("MOVIDO COM SUCESSO!"); }
+function trocarPgto(id, status) { enfileirarOuExecutar({ type: 'update', collection: 'pedidos', docId: id, data: { statusPagamento: status } }); }
+function excluirPedido(id) { if (confirm("Deletar pedido inteiro?")) enfileirarOuExecutar({ type: 'delete', collection: 'pedidos', docId: id }); }
+
+// ==========================================
+// BULK UPDATE E FATURAMENTO
+// ==========================================
...
+    const ops = selecionados.map(id => ({ type: 'update', collection: 'pedidos', docId: id, data: { status: status } }));
+    enfileirarOuExecutar({ type: 'batch', ops }).then(() => { showToast(`${selecionados.length} PEDIDOS ATUALIZADOS!`); limparBulk(); });
+}
+
+function atualizarSelectFaturamento(mesesUnicos) {
+    const select = document.getElementById('selectFaturamentoMes'); const valAnt = select.value; select.innerHTML = '';
...
+    });
+    if(itens.length === 0) { showToast("Precisa de pelo menos 1 peça.", true); return; }
+    enfileirarOuExecutar({ type: 'update', collection: 'pedidos', docId: id, data: {
+        nome: document.getElementById('editNome').value.toUpperCase(), whatsapp: document.getElementById('editWhatsapp').value,
+        valorTotal: unmaskCurrency(document.getElementById('editValorTotal').value), metodoPagamento: document.getElementById('editMetodoPagamento').value, itens: itens 
+    } }).then(() => { fecharModalEdicao(); showToast("Pedido Editado!"); });
+}
