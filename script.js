...
function preencherDadosClienteNoLancamento(perfil = {}, compras = {}) {
    ...
    if(enderecoCompleto.includes(',')) {
        ...
        } else if(ultimaParte.toUpperCase().startsWith('COMPL:')) {
            complemento = ultimaParte.replace(/COMPL:/i, '').trim();
            let penultima = (partes.pop() || '').trim();
            if(/^\d+[A-Za-z-]*$/.test(penultima)) numero = penultima;
            else if(penultima) partes.push(penultima);
            enderecoBase = partes.join(',').trim();
        }
    }
    ...
    document.getElementById('complementoEnd').value = perfil.complemento || compras.complemento || complemento || '';
}
...
async function salvarPedidoCompleto() {
    ...
    let complemento = document.getElementById('complementoEnd').value.trim();
    let end = document.getElementById('endereco').value + ", " + document.getElementById('numeroEnd').value + (complemento ? `, COMPL: ${complemento}` : '');
    let frete = unmaskCurrency(document.getElementById('valorFrete').value);
    ...
    try {
        await db.collection("pedidos").add({
            numeroPedido: numGerado, nome: nome, whatsapp: whatsapp, cep: cep, endereco: end, complemento: complemento, valorFrete: frete,
        ...
        db.collection("clientes").doc(whatsapp).set({ whatsapp: whatsapp, nome: nome, cep: cep, endereco: end, complemento: complemento }, { merge: true });
...
db.collection("pedidos").orderBy("dataCriacao", "desc").onSnapshot((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
        ...
        if(p.whatsapp) {
            if(!mapaClientes[p.whatsapp]) mapaClientes[p.whatsapp] = { nome: p.nome, totalGasto: 0, qtd: 0, cep: p.cep, endereco: p.endereco, complemento: p.complemento, ultimaCompra: d, pedidos: [] };
