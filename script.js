...
if(!isVitrine) { 
    ...
    db.collection("transacoes_manuais").onSnapshot(snap => { 
        ...
        if(document.getElementById('modalFluxoCaixa') && document.getElementById('modalFluxoCaixa').style.display === 'flex') renderizarFluxoCaixa();
    });
}
...
function renderizarFluxoCaixa() {
    let filtro = document.getElementById('fcFiltroMes').value; 
    if(!filtro) return;
    let [anoFiltro, mesFiltro] = filtro.split('-');
    let mesAnoFiltro = `${mesFiltro}/${anoFiltro}`; 

    let extrato = []; let totalEntradas = 0; let totalSaidas = 0;

    // 1. Puxar Vendas Automáticas (Entradas)
    todosPedidos.forEach(p => {
        if(p.statusPagamento === 'PAGO') {
            let d = p.dataCriacaoSafe || new Date();
            let mesAnoPed = `${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
            if(mesAnoPed === mesAnoFiltro) {
                let valor = safeNum(p.valorTotal);
                extrato.push({ dataObj: d, dataFmt: d.toLocaleDateString('pt-BR'), desc: `Venda #${p.numeroPedido} (${p.nome.split(' ')[0]})`, origem: 'SISTEMA (VENDA)', tipo: 'ENTRADA', valor: valor });
                totalEntradas += valor;
            }
        }
    });

    // 2. Puxar Despesas Fixas Automáticas (Saídas da DRE)
    despesasGlobais.forEach(d => {
        if(d.mesAno === mesAnoFiltro) {
            let valor = safeNum(d.valor);
            extrato.push({ dataObj: new Date(anoFiltro, parseInt(mesFiltro)-1, 1), dataFmt: `Ref: ${mesAnoFiltro}`, desc: d.descricao, origem: 'SISTEMA (CUSTO FIXO)', tipo: 'SAIDA', valor: valor });
            totalSaidas += valor;
        }
    });

    // 3. Puxar Lançamentos Manuais
    transacoesManuais.forEach(t => {
        let [anoT, mesT, diaT] = t.dataIso.split('-');
        if(anoT === anoFiltro && mesT === mesFiltro) {
            let valor = safeNum(t.valor);
            extrato.push({ dataObj: new Date(anoT, parseInt(mesT)-1, diaT), dataFmt: `${diaT}/${mesT}/${anoT}`, desc: t.descricao, origem: 'MANUAL', tipo: t.tipo, valor: valor, id: t.id });
    ...
    if(extrato.length === 0) {
        ...
        tbody.innerHTML = extrato.map(item => {
            ...
            return `
            <tr style="border-bottom: 1px solid var(--border-color); background: var(--white);">
                <td style="padding: 10px; font-weight: 800;">${item.dataFmt}</td>
                <td style="font-weight: 900;">${item.desc}</td>
                <td style="font-size: 0.7rem; color: var(--text-muted); font-weight: 800;">${item.origem}</td>
                <td style="text-align: right; color: ${cor}; font-weight: 900; padding-right: 10px; font-size: 1.1rem;">${sinal} ${formatCurrency(item.valor)}</td>
                <td style="text-align: center;">${btnExcluir}</td>
            </tr>`;
        }).join('');
    }

    document.getElementById('fcEntradas').innerText = formatCurrency(totalEntradas);
    document.getElementById('fcSaidas').innerText = formatCurrency(totalSaidas);
    document.getElementById('fcSaldo').innerText = formatCurrency(totalEntradas - totalSaidas);
}
...
function salvarTransacaoManual(e) {
    ...
    db.collection("transacoes_manuais").add({ tipo: tipo, descricao: desc, valor: valor, dataIso: dataIso, dataCriacao: firebase.firestore.FieldValue.serverTimestamp() })
    .then(() => {
        document.getElementById('fcDesc').value = ''; document.getElementById('fcValor').value = '';
        showToast("Lançamento adicionado no Caixa!");
    });
}

function deletarTransacaoManual(id) {
    if(confirm("Deseja apagar este lançamento manual permanentemente?")) {
