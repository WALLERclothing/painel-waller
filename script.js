...
async function cotarFrete() {
    ...
    try {
        let fetchUrl = 'https://corsproxy.io/?' + encodeURIComponent('https://www.melhorenvio.com.br/api/v2/me/shipment/calculate?t=' + Date.now());
        ...
        lista.innerHTML = '<div style="text-align:center; color:var(--red); padding:20px; font-weight:900;">⚠️ Bloqueio de Conexão: Desative seu AdBlocker ou Escudo do Brave nesta página. Eles bloqueiam a conexão com a Transportadora.</div>';
    }
}

async function chamarApiCarrinhoME(payload) {
    let fetchUrl = 'https://corsproxy.io/?' + encodeURIComponent('https://www.melhorenvio.com.br/api/v2/me/cart?t=' + Date.now());
    let res = await fetch(fetchUrl, {
        method: 'POST',
        headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'Authorization': `Bearer ${ME_TOKEN}` },
        body: JSON.stringify(payload)
...
async function enviarParaMelhorEnvio(pedidoId) {
    ...
    try {
        ...
        alert("⚠️ Falha ao se conectar com o servidor do MelhorEnvio. Desative seu AdBlocker nesta aba e tente novamente.");
    }
}
...
// ==========================================
// 🚀 INJEÇÃO MÁGICA NO CARRINHO DO MELHOR ENVIO (COM FALLBACK INTELIGENTE)
// ==========================================
async function chamarApiCarrinhoME(payload) {
    let fetchUrl = 'https://corsproxy.io/?' + encodeURIComponent('https://www.melhorenvio.com.br/api/v2/me/cart?t=' + Date.now());
    let res = await fetch(fetchUrl, {
